...
export default function DashboardPage() {
  ...
  if (loading) return <div className="card">Loading run history…</div>;
  if (loadError) return <div className="card error-box">Could not load run history: {loadError}</div>;
  if (runs.length === 0) return <div className="card">No runs yet. Process an invoice in the Run tab to see it here.</div>;
